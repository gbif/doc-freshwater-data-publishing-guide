...
* *Opportunistic observation data*: unplanned observations that are not part of a systematic sampling event, but that occur as circumstances allow. Specific effort is not made to observe or collect particular species or an <<assemblage>> of species, and no sampling protocol is used. + _Example_: data originating from bird watching or records from iNaturalist or similar apps.
...
[cols="18,25,25,12,~"]
|===
| Term |Freshwater definition |Freshwater example(s) |Freshwater status |Freshwater comment
...
|dataLanguage
|lLnguage in which the data is provided
^|"English" +
"German"
|Required
|
