...
Freshwater datasets that are submitted through a GBIF node or uploaded using https://www.gbif.org/ipt[GBIF Integrated Publishing Toolkit^] (IPT) software should therefore be tagged as “freshwater” to make the dataset more visible to the freshwater community. This can be done by allocating the specific dataset to the “Freshwater Network” during the publication process, after registering it with GBIF.
