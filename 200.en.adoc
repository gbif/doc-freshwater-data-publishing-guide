...
Checklist datasets can be used to make it easier to select all freshwater assemblage data from a particular region, if a checklist of the freshwater species in the region is available. For more information on the possible types of checklists and application of checklists, see https://data-blog.gbif.org/post/gbif-checklist-datasets-and-data-gaps/[GBIF Checklist Datasets and Data Gaps^].
...
|Required, though year, month, day, or https://techdocs.gbif.org/en/data-processing/temporal-interpretation#occurrence-recorded-date[other terms could be used instead^] 
...
[[table-07]]
.Terms, definitions, examples, and comments recommended for inclusion with freshwater datasets. The dataset in which each field should be included (metadata, occurrence or event) is indicated, as is whether fields are required, recommended, or share if relevant on particular organism groups (https://dwc.tdwg.org/list/[more information^] on the specific fields).
...
|One of `fungi`,`microbes`,`benthic algae`,`phytoplankton`,`macrophytes`,`zooplankton`,`benthic macroinvertebrates`,`decapods`,`fish`,`amphibians`,`reptiles`,`birds`,`mammals`
|Required
...
|Recommended
|Recommended best practice is to separate the values in a list with space vertical bar space, or post (`\|`). The primary collector or observer should be listed first. 
|Occurrence
